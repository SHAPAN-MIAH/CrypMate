...
import { faChevronLeft, faCloudUploadAlt, faTimes } from '@fortawesome/free-solid-svg-icons';
...
const ConfirmSecretBackupPhrase = () => {
  ...
  const confirmDataInputHandler = (e) => {
    ...
    const confirmSecretBackupPhraseBtnOverlay = document.querySelector(".confirmSecretBackupPhrase-btn-overlay");
    const ConfirmThroughEncrypt = document.querySelector(".Confirm-through-encrypt");
    if(phraseDataItem) {
  ...
  return (
    <>
      <div className="container-fluid" style={{width: "950px", margin: "auto"}}>
        <div className="header" style={{marginBottom: '-30px', marginTop: '-20px'}}>
             <img src={logo} alt=""/> 
             <h2>CrypMate</h2>
        </div>
        <span style={{margin: "0 45px",cursor: "pointer", fontFamily: "Poppins", color: "color: rgb(58, 58, 58)", marginBottom: "40px"}} onClick={() => goBack()}><FontAwesomeIcon icon={faChevronLeft} /> Back</span>
        ...
        <div className="confirm-secret-backup-phrase-container">
          ...
          <div className="confirm-secret-backup-phrase-title">
            ...
            <p>Select each word by the order they appear in your seed phrase to confirm, or 
              <br/> 
              <button onClick={openModal}>Confirm with downloaded encryption file</button> 
            </p>
          </div>
          ...
          <div className="confirm-phrase-input-field mb-3 mt-4">
              ...
              <div className="Confirm-through-encrypt">
                <p>Confirm through <span onClick={openModal}>encrypted file</span></p>
              </div>
          <div id="confirmPhraseButtonContainer" className="confirm-phrase-button-container">
            {
              Phrases.map((phrase, index) => {
                return(
                  <button onClick={confirmDataInputHandler} id="dataText" type="">{phrase}</button>
                )
              })
            }
          </div>
          
          <button onClick={() => goTo(Congratulations)} id="confirmSecretBackupPhrase-btn" type="">confirm</button>
          <br/><span className="confirmSecretBackupPhrase-btn-overlay" ></span>
        </div>
